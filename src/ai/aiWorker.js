...
- Ставь manager_issues только когда проблема очевидна из сообщений менеджера. Если это спорно — оставь пустой массив.
- Не ругай закрытые/логистические диалоги: финальные "ок", "спасибо", "хорошо", подтверждение адреса/доставки/оплаты/чека сами по себе НЕ являются no_response/no_followup.
- slow_response = менеджер ответил дольше 30 минут рабочего времени; дольше 1 часа считается критично.
- no_response = последнее содержательное сообщение клиента содержит вопрос/просьбу и менеджер ещё не ответил. Не ставь на финальные короткие подтверждения клиента.
- no_followup = клиент явно взял паузу/сравнивает/думает после КП или цены, прошло >24 часов, и менеджер не вернулся с повторным касанием. Не ставь, если сделка уже в payment/delivery/completed/refused.
- short_template_only = менеджер ответил коротко/шаблонно без вопроса, без КП/фото/прайса и без конкретного следующего шага. Не ставь, если менеджер дал срок, доставку, оплату, адрес, КП или другой понятный next step.
- poor_consultation = в активной продаже менеджер не выяснил критичные параметры, из-за чего невозможно нормально подобрать товар. Не ставь на сервис, доставку, гарантию или проверку статуса заказа.
- no_photos = клиент прямо попросил фото/видео/каталог, а после этой просьбы менеджер не отправил image/video/document/каталог/КП. Если отправил видео или документ после просьбы — не ставь.
- no_showroom_invite = только для ранней активной продажи конечному клиенту, который выбирает кухонную мойку/смеситель/измельчитель и может посетить шоурум в Астане/Алматы. Не ставь на partner/designer/dealer, payment/delivery/service/existing customer, запросы про ванную/туалет/тумбу, передачу контакта другому менеджеру/городу, и не ставь если уже дан адрес/2GIS/приглашение/самовывоз, клиент уже был/собирается в шоурум, или менеджер дал другой понятный следующий шаг.
- no_upsell = только если клиент покупает основной товар, менеджер уже дал цену/КП, но вообще не предложил релевантные сопутствующие товары. Не ставь на покупку одного аксессуара, сервис, гарантию, доставку или если клиент отказался от доп. товара.
- rude_tone = грубый или неприветливый тон именно менеджера. Жалоба клиента на другого человека не равна rude_tone в этой переписке, если менеджер сам не грубил.
- formal_tone = слишком сухой/формальный ответ без эмпатии в ситуации, где клиенту нужна персональная помощь. Не дублируй с short_template_only без причины.
- no_alternative = менеджер сказал "нет", "нет в наличии", "не бывает" и не предложил аналог/альтернативу/следующий путь.
- manager_issues = пустой массив если менеджер работает нормально или проблема не доказана.
