...

{
  ...
  "customer_type": "одно из: end_client, partner, colleague, unknown",
...
// Map AI customer_type → tag name (Russian) — simplified to 4 tags
const CUSTOMER_TYPE_TAG = {
  ...
  spam: 'неизвестно',       // spam → unknown for now
  unknown: 'неизвестно',
};
...
async function applyAutoTag(sessionId, remoteJid, customerType) {
  ...
  try {
    ...
    const employeePatterns = ['OMOIKIRI', 'ОМОЙКИРИ', 'OMOIKIRI ALMATY', 'OMOIKIRI ASTANA'];
    const isEmployee = employeePatterns.some((p) => chatName.includes(p));
...
- first_contact = первое обращение, реального обсуждения ещё нет
- consultation = обсуждение потребностей, материалов, размеров
- model_selection = активный подбор конкретной модели/продукта
- price_negotiation = обсуждение цены, скидок, условий оплаты
- payment = процесс оплаты, выставление счёта
- delivery = доставка, планирование установки
- completed = сделка завершена, товар доставлен
- refused = клиент отказался или перестал отвечать
- needs_review = невозможно определить по сообщениям
            ...
            if (idx >= 0 && idx < batch.length && r.customer_type && CUSTOMER_TYPE_TAG[r.customer_type]) {
              ...
              if (r.deal_stage && VALID_STAGES.includes(r.deal_stage)) {
                try {
                  ...
                  if (latest) {
                    // Only update if current stage is default/unknown — don't override manual or daily-analysis stages
                    if (['first_contact', 'needs_review'].includes(latest.deal_stage)) {
                      await supabase.from('chat_ai').update({ deal_stage: r.deal_stage }).eq('id', latest.id);
                    }
                  } else {
