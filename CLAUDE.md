...
## ОБЯЗАТЕЛЬНОЕ ЧТЕНИЕ ПЕРЕД РАБОТОЙ С БД

`ObsidianVault/projects/omoikiri/data-schema.md` — canonical reference для всех таблиц / колонок / enum / связей / API payloads (~32 таблицы, 8+ enum'ов, ER diagram, snake↔camel boundary, 10 gotchas).

Если ты собираешься:
- писать SQL / миграцию,
- использовать `.from('X').select('Y')` или фильтровать колонку,
- маппить frontend поле к backend колонке (snake_case ↔ camelCase),
- работать с lead_source / customer_type / deal_stage / tags / risk_flags / manager_issues / category и т.д.,

— **RTFM сначала**. Не угадывай. Прошлые баги (например, `partner_contacts.phone` vs `primary_phone` 2026-05-04 audit C-1) стоили дня дебага.

При изменении схемы — **обнови `data-schema.md` в той же PR**.
